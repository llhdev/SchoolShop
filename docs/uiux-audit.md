...
> **Pass 2 update (2026-09-03):** B1, B2, B4, B5 (critical blockers) and M1, M2, M5, M7 (medium) are now **implemented**. Details below remain as originally diagnosed; the "Proposed solution" sections for these items describe what was built. Still open: **M3** (denormalized cart lines), **M6** (startup request count — partially improved by B5's order gating), **M8** (optimistic rollback snapshots), **M9** (cache parse failure signal). M4 and M7 were absorbed into B1 (expo-image transitions + local placeholder).
...
**Status legend:** ✅ implemented in pass 1 · ✅✅ implemented in pass 2 · ⬜ open
...
- **Proposed solution (as implemented in pass 2):** All product surfaces render through `ProductImage`, a wrapper around `expo-image` (memory + disk cache, fade-in). Uploads store a 400px `-thumb.jpg` companion alongside the 800px original (naming convention — no schema change, works on any Supabase plan); list views fetch the derived thumbnail and fall back to the full URL on error for products uploaded before thumbnails existed. Missing images render a local category-colored initial placeholder (also implements M7).
...
**Pass 3 update (2026-09-03): the Mini App is implemented.** The web build now runs as a Telegram Mini App: `src/lib/telegram.ts` wraps the official SDK (lazy-loaded, no-ops outside Telegram), with theme sync, safe-area insets, BackButton wired to navigation, MainButton as the checkout CTA, and the RN header hidden in-TMA. Shopper identity runs through the `telegram-auth` Edge Function (validated initData → `shoppers` table, migration 011) and order history follows the Telegram account. Admins work in the WebView via the web image pipeline (`src/lib/image-web.ts`: file input + canvas resize); native-only modules are dynamically imported. Deploy: GitHub Pages via `npm run deploy:web` / `.github/workflows/deploy-web.yml`. Full details in `docs/telegram-mini-app.md`. The structural notes below remain for context:

1. **Platform gates all flip to "web" inside a TMA WebView** — ~30 `Platform.OS === 'web'` checks across 19 files. ~~Mostly good (responsive grids, WebHeader), but the web bottom tab bar is **hidden** (`UserTabNavigator.tsx:23`), so Cart/Orders have no navigation chrome unless every screen renders a web header.~~ ✅ Closed in pass 3: the bottom tab bar now renders on web (thumb-friendly), `WebHeader` is desktop-only.
2. **No `linking` config** on `NavigationContainer` (`AppNavigator.tsx:19`) — ~~no deep links today. Add when you want shareable product/order links; also needed to cooperate with Telegram's native BackButton semantics (`history.back()` / close Mini App).~~ ✅ Closed in pass 3: `linking` config with `product/:productId`, `cart`, `orders`, `checkout`, `order/:orderId` (GitHub Pages base path stripped), and BackButton drives the stack.
3. **Admin-only native modules won't exist in a TMA** — ~~`expo-image-picker`, `expo-image-manipulator`, `@react-native-picker/picker`, `expo-file-system` are statically imported by admin screens/utils and land in the bundle. Plan code-splitting (dynamic imports behind role checks) or replace with Telegram attachment APIs. Shopper flows are unaffected.~~ ✅ Closed in pass 3: image picking/resizing on web uses a hidden file input + canvas; the native modules are dynamically imported so they drop out of the web bundle.
...
6. **Viewport/safe-area**: `Screen` uses safe-area-context which maps to CSS `env(safe-area-inset-*)` on web — ~~correct for notched phones in a WebView; verify the always-on `bottom` edge against your in-TMA navigation once built.~~ ✅ Closed in pass 3: `Screen` adds Telegram's `contentSafeAreaInset.bottom` inside the Mini App.
7. **`react-native-share`** is native-only and already dynamically imported — the web path uses `navigator.share`; ~~inside Telegram, prefer sharing via `t.me` links or the Bot API instead.~~ ✅ Closed in pass 3: TMA share uses a `t.me/share/url` link.

---

## Suggested order for the next pass (updated after pass 2)

1. M3 — denormalize cart lines (store `productId`, re-derive from products; merge on hydrate for deleted products).
2. M6 — trim startup requests further (lazy/deferred fetches; column selection on product lists).
3. M8 — capture optimistic-update snapshots in the reducer instead of render closures.
4. M9 — cache parse-failure logging/versioning.
5. TMA readiness: per-screen web navigation chrome (tab bar is hidden on web), `linking` config, admin-module code splitting.
